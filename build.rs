...
fn build(sdk_path: Option<&str>, target: &str) {
    ...
    if let Some(sdk_path) = sdk_path {
        ...
        for framework in get_frameworks(sdk_path.clone()) {
            ...
            builder = builder.header_contents(&format!("{}.h", framework), &meta_header.concat());
            ...
            bindings
                .write_to_file(&format!("src/{}.rs", framework))
                //.write_to_file(out_dir.join(&format!("{}.rs", framework)))
